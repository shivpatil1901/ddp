#!/usr/bin/env python3
"""Evaluate IQ-Learn PointGoal policy and save reward/cost statistics.
...
# this script directly (python iql/eval_iql_pointgoal.py).
_HERE = os.path.dirname(os.path.abspath(__file__))
_PROJECT_ROOT = os.path.abspath(os.path.join(_HERE, '..'))
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from iql.iql_run import (
...
def evaluate(policy_agent, env, n_episodes=500, deterministic=True, verbose=False):
    ...
    for ep in range(n_episodes):
        ...
        if verbose:
            print(f"Episode {ep+1}/{n_episodes}: reward={ep_reward:.2f}, cost={ep_cost:.2f}")
...
def main():
    ...
    parser.add_argument('--checkpoint', default='iq_learn_pointgoal_best.pth')
    ...
    out_path = os.path.join(args.save_dir, f'iql_eval_pointgoal_{ts}.json')
    with open(out_path, 'w') as f:
        json.dump(summary, f, indent=2)

    print('Evaluation complete:')
    print(json.dumps({k: summary[k] for k in ('env','checkpoint','episodes','reward_mean','reward_std','cost_mean','cost_std','time_s')}, indent=2))
