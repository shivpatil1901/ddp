...
def run_polopt_agent(env_fn, 
                     ...
                     save_critic=True,
                     save_critic_freq=1,
                     critic_checkpoint_dir='critic_checkpoints',
    ...
    # Setup dedicated critic checkpoint saving (value networks only).
    critic_saver = None
    critic_save_root = None
    critic_vars = get_vars('vf')
    if not(agent.reward_penalized):
        critic_vars += get_vars('vc')

    if save_critic and proc_id() == 0:
        critic_save_root = os.path.join(logger.output_dir, critic_checkpoint_dir)
        os.makedirs(critic_save_root, exist_ok=True)
        critic_saver = tf.train.Saver(var_list=critic_vars, max_to_keep=1000000)
        logger.log('Critic checkpoints will be saved in: %s' % critic_save_root, color='cyan')

    def maybe_save_critic(epoch_idx):
        if not save_critic:
            return
        if proc_id() != 0:
            return
        if critic_saver is None or critic_save_root is None:
            return

        local_epoch = epoch_idx - start_epoch
        is_last = (epoch_idx == (start_epoch + epochs - 1))
        if (local_epoch % save_critic_freq != 0) and (not is_last):
            return

        ckpt_name = 'critic_epoch_%06d.ckpt' % int(epoch_idx)
        ckpt_path = os.path.join(critic_save_root, ckpt_name)
        critic_saver.save(sess, ckpt_path)
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        # Save critic/value function checkpoint after update for this epoch.
        maybe_save_critic(epoch)
