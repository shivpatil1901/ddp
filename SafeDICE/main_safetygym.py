...
parser.add_argument('--pretrain_iters', type=int, default=100000)
...
if len(gpus) > 0:
    ...
    print(f"✅ GPUs found: {len(gpus)} device(s), memory growth enabled on all")
else:
...
def load_pickle_compat(path):
    try:
        ...
        if 'unsupported pickle protocol: 5' in str(e):
            ...
            raise RuntimeError(
                f"Failed to load {path}: pickle protocol 5 requires Python >= 3.8 or the pickle5 backport. "
                f"Current Python is {py_ver}. Install pickle5 in this environment (e.g., pip install pickle5), "
                f"or run with Python >= 3.8."
            ) from e
        raise

# Load anti_expert demonstrations (ppo)
anti_expert_path = './dataset/safetygym/ppo_{}{}_s0.pickle'.format(robot_name, task_name)
print(f'Loading anti-expert dataset from {anti_expert_path}', flush=True)
anti_expert_demo = load_pickle_compat(anti_expert_path)
if sys.version_info < (3, 8) and not HAS_PICKLE5:
    print('Warning: Running on Python < 3.8 without pickle5; protocol 5 pickles may fail to load.')
...
expert_path = './dataset/safetygym/ppo_lagrangian_{}{}_s0.pickle'.format(robot_name, task_name)
print(f'Loading expert dataset from {expert_path}', flush=True)
expert_demo = load_pickle_compat(expert_path)
...
print('Finished dataset loading. Initializing model...', flush=True)
...
if algorithm in ['antidice', 'dwbc']:
    ...
    print(f'Starting discriminator pretraining for {args.pretrain_iters} iterations...', flush=True)
    while pretrain_iter < args.pretrain_iters:
        ...
        pretrain_iter += 1
        if pretrain_iter % 100 == 0:
            print('Pretrain iter: {}'.format(pretrain_iter), flush=True)
